...
def _epoch_iterator(
    ...
    target_fs: int,
    epoch_labels: Optional[List[str]] = None,
):
    """
    Yield (idx, epoch_data [n_chan x n_samples float32], label_str).

    - Uses fixed-length 30s epochs over the entire recording.
    - Resamples/decimates to target_fs.
    - Aligns each epoch with a label from epoch_labels, if provided.
      If epoch_labels is shorter, it pads with the last label and logs a warning.
      If epoch_labels is None, all labels default to "Wake".
    """
    orig_fs = raw.info["sfreq"]
    events = mne.make_fixed_length_events(raw, duration=chunk_duration)

    # If target_fs divides orig_fs nicely, use decimation
    decim = None
    ...
    # Sanity: compare EDF epochs vs TXT labels
    if epoch_labels is not None:
        n_labels = len(epoch_labels)
        if n_labels != n_events:
            logger.warning(
                f"EDF epochs ({n_events}) vs TXT-derived labels ({n_labels}) mismatch. "
                f"Will use min(n_events, n_labels) and pad with last label if needed."
            )

    warned_padding = False

    for i in range(n_events):
        x = epochs[i].get_data()[0]
        if decim is None and target_fs != orig_fs:
            new_len = int(round(x.shape[1] * (target_fs / orig_fs)))
            x = scipy_resample(x, num=new_len, axis=1)
        yield i, x.astype(np.float32), None  # we’ll handle labels outside
...
def main():
    ...
    for edf in files:
        ...
        # Locate matching TXT file
        txt_path = os.path.join(os.path.dirname(edf), f"{base}.txt")
        epoch_labels = None
        if os.path.exists(txt_path):
            logger.info(f"Found TXT scoring file for {base}: {os.path.basename(txt_path)}")
            epoch_labels = _parse_cap_txt_file(txt_path, chunk_duration=args.chunk_duration)
        else:
            logger.warning(f"No TXT scoring file found for {base}; using dummy 'Wake' labels.")
        ...
        try:
            # Generate all epochs first (without saving yet)
            epochs_iter = list(
                _epoch_iterator(
                    raw,
                    args.chunk_duration,
                    args.target_sampling_rate,
                    epoch_labels=None,  # we’ll manually assign labels below
                )
            )

            n_epochs_edf = len(epochs_iter)
            n_labels = len(epoch_labels) if epoch_labels is not None else 0
            n_use = min(n_epochs_edf, n_labels)

            if n_epochs_edf != n_labels:
                logger.warning(
                    f"Epoch/label mismatch for {base}: EDF epochs={n_epochs_edf}, TXT labels={n_labels}. "
                    f"Using first {n_use} pairs and dropping the rest."
                )
        ...
        if len(labels_dict) > 0:
            ...
            logger.info(
                f"Saved {len(labels_dict)} epochs for {base} "
                f"(EDF epochs: {n_epochs_edf}, label entries: "
                f"{len(epoch_labels) if epoch_labels is not None else 'dummy/Wake'})"
            )
        else:
